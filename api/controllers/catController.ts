...
export class CatController extends Base {
     ...
     */
    async getCats(breed: string, limit: number): Promise<CatBreeds> {
        const response = await this.request.get(`https://api.thecatapi.com/v1/images/search?limit=${limit}&breed_ids=${breed}&api_key=${process.env.API_KEY}`);
